...
const forwarderOrigin = currentUrl.hostname === 'cryptoknightchess.com'
  ? 'http://www.cryptoknightchess.com'
  : undefined
  ...
  const updateButtons = () => {
    ...
    if (accountButtonsDisabled) {
      for (const button of accountButtons) {
       ...
       // try{console.log(button)}catch(ex){console.log(ex)}
      }
      clearTextDisplays()
    } else {
      try{
      ...
      catch(ex){console.log("nothing to disable" + ex)}
    }
  ...
  const initializeAccountButtons = () => {
    ...
    getAccountsButton.onclick = async () => {
      try {
        ...
        console.log("it's " + getAccountsResults.innerHTML);
